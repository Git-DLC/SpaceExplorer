...
def getGameOverMenu(rootMenu, screen_width, screen_height):
    ...
    menu = pygame_menu.Menu('Конец игры', screen_width, screen_height, theme=mytheme)

    framy = menu.add.frame_v(width=screen_width * 0.6, height=screen_height * 0.6,
                             background_color=(255, 165, 0, 100)
                             )

    framePreset(framy, widgetPreset(menu.add.button('Играть снова', source.game.playGame,
                                                    rootMenu, screen_width, screen_height)))
    framePreset(framy, widgetPreset(menu.add.button('Выйти в меню', menu.disable)))
